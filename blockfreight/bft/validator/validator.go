...
func ValidateBoL(jsonpath string, printJson bool) string {
    //printJson := true
    //examplePath := "./files/bf_tx_example.json"
    espErr := ""
    var bol bol.BoL
    json.Unmarshal(ReadJSON(jsonpath), &bol)
    if printJson { spew.Dump(bol) }
    valid, err := ValidateFields(bol)
    if valid {
        return "Success! [OK]"
    } else {
        if err != "" {
...
func ValidateFields(bol bol.BoL) (bool, string){
