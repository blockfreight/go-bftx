...
do
    ...
    ssh -oStrictHostKeyChecking=no $BLOCKFREIGHT_SSH_USER@$validator "env validator_name=$validator_name;curl https://raw.githubusercontent.com/blockfreight/tools/master/blockfreightnet-kubernetes/examples/blockfreight/app.yaml > app.yaml;sed -i -- 's/<VALIDATOR_NAME>/$validator_name/g' app.yaml && sed -i -- 's|'$CONTAINER'|'blockfreight/go-bftx:$1'|g' app.yaml;echo old_tag: $CONTAINER;echo tag: $1;cat app.yaml;kubectl apply -f app.yaml && kubectl delete pods --all --grace-period=0 --force;rm app.yaml pre_app.yaml" <<-'ENDSSH'
